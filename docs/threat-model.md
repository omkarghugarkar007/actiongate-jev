...
| Caller fabricates or omits RBAC or resource facts | Hard rules take precedence over semantic evidence; a rule whose fact is absent blocks rather than passing; and a tool marked `requireTrustedFacts` accepts only facts resolved server-side by a configured provider, with `maxFactAgeSeconds` rejecting stale ones | `requireTrustedFacts` is opt-in per tool, so a policy that omits it still accepts caller-asserted facts. Fact providers are only as trustworthy as the services behind them, and those services must not be reachable by the agent |
