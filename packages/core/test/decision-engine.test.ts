...
import { AuthorizationEngine, DEFAULT_POLICY, FunctionFactProvider, actionFingerprint, buildMinimalState, failureDecisionForRisk, redactSecrets, type AuthorizationEngineOptions, type AuthorizationRequest } from "../src/index.js";
...
const engine = (
  ...
  facts: AuthorizationRequest["deterministicFacts"] = request().deterministicFacts,
  options: AuthorizationEngineOptions = {}
) => new AuthorizationEngine(provider, {
  ...options,
  factProviders: [new FunctionFactProvider({ name: "test-system", resolve: () => facts })]
});

describe("AuthorizationEngine", () => {
  it("allows a supported action", async () => expect((await engine(FakeDecisionProvider.allow()).authorize(request(), DEFAULT_POLICY)).decision).toBe("ALLOW"));
  it("records provider-reported token usage and cost", async () => {
    const result = await engine(FakeDecisionProvider.allow()).authorize(request(), DEFAULT_POLICY);
  ...
  it("blocks an exact numeric limit violation before Jev", async () => {
    ...
    const facts = { authenticated: true, authorizedByRbac: true, duplicate: false, amountCents: 49_000, currency: "USD" } as const;
    const result = await engine(provider, facts).authorize(request({ deterministicFacts: facts }), DEFAULT_POLICY);
    expect(result.decision).toBe("BLOCK");
    expect(result.reasons[0]?.code).toBe("AMOUNT_EXCEEDS_LIMIT");
    expect(provider.calls).toBe(0);
  });
  it("blocks semantic scope expansion", async () => expect((await engine(FakeDecisionProvider.scopeExpansion()).authorize(request(), DEFAULT_POLICY)).decision).toBe("BLOCK"));
  it("routes missing intent to review", async () => expect((await engine(FakeDecisionProvider.missingIntent()).authorize(request(), DEFAULT_POLICY)).decision).toBe("REVIEW"));
  it("fails closed for financial provider errors", async () => expect((await engine(FakeDecisionProvider.error()).authorize(request(), DEFAULT_POLICY)).decision).toBe("BLOCK"));
  it.each([
    ...
    const provider = FakeDecisionProvider.allow();
    const result = await engine(provider, deterministicFacts).authorize(request({ deterministicFacts }), DEFAULT_POLICY);
    expect(result.decision).toBe("BLOCK");
    expect(result.reasons.some((reason) => reason.code === code)).toBe(true);
    expect(provider.calls).toBe(0);
  });
  it("blocks an agent risk downgrade", async () => {
    const result = await engine(FakeDecisionProvider.allow()).authorize(request({ proposedAction: { ...request().proposedAction, riskClass: "READ_ONLY" } }), DEFAULT_POLICY);
  ...
  it("reviews read-only provider failures by default and explicitly supports fail-open", async () => {
    ...
    expect((await engine(FakeDecisionProvider.error()).authorize(read, DEFAULT_POLICY)).decision).toBe("REVIEW");
    expect((await engine(FakeDecisionProvider.error(), request().deterministicFacts, { failOpenReadOnly: true }).authorize(read, DEFAULT_POLICY)).decision).toBe("ALLOW");
  });
  it("reports but does not enforce in shadow mode", async () => {
    const result = await engine(FakeDecisionProvider.scopeExpansion()).authorize(request({ mode: "shadow" }), DEFAULT_POLICY);
